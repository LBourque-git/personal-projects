function[Temp_matrix_initial, Temp_at_point] = central_temp_solver(Temp_matrix_initial,num_rows,num_columns, alpha, Delta_x, Delta_y, h_a, k_a) %Solves the temperatures for the transient case using the explicit Euler method
...
for t = 0.0001:h:4
    ...
    for lv_column = 2:num_columns-1
        for lv_row = num_rows-1:-1:2
            Temp_Central(lv_row, lv_column) = Explicit_euler(Temp_matrix_initial, lv_row, lv_column, h, Delta_x, Delta_y, alpha); %Solves for the central temperatures using explicit Euler as the IVP solver
        end
    end
    ...
    New_Temp_matrix_initial(1,:) = top_temp_solver(New_Temp_matrix_initial, h_a, k_a, Delta_y, num_columns); %Find the top boundary temperature
    Temp_matrix_initial = New_Temp_matrix_initial;
